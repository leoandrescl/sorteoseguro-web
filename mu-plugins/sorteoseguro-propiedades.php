<?php
/**
 * Plugin Name: Sorteo Seguro – Propiedades
 * Description: Plantilla PHP para la página Propiedades (ID 99), con chrome header/footer.
...
final class SorteoSeguro_Propiedades {
	...
	public static function is_propiedades_page(): bool {
		return is_page(self::PAGE_ID);
	}

	public static function enable_chrome(bool $enabled): bool {
		return self::is_propiedades_page() ? true : $enabled;
	}

	public static function bypass_page_cache(): void {
		if (!self::is_propiedades_page()) {
			return;
		}
		do_action('litespeed_control_set_nocache', 'ss-propiedades');
		if (!headers_sent()) {
			nocache_headers();
	...
	public static function template_include(string $template): string {
		...
		$custom = WP_CONTENT_DIR . '/mu-plugins/sorteoseguro-propiedades/page-propiedades.php';
	...
	public static function assets(): void {
		...
		$base      = content_url('mu-plugins/sorteoseguro-propiedades/assets');
		$dir       = WP_CONTENT_DIR . '/mu-plugins/sorteoseguro-propiedades/assets';
		$ver       = self::VERSION;
		...
		if (is_readable($dir . '/propiedades.css')) {
			wp_enqueue_style('ss-propiedades', $base . '/propiedades.css', ['ss-home'], $ver);
		}
	...
	public static function inline_css(): void {
		...
		echo "\n<!-- ss-propiedades v" . esc_html(self::VERSION) . " -->\n";
		$files = [
	...
	/**
	 * Hero inmobiliario: Casa + Parcela (mismos slides del home).
	 *
	...
	/**
	 * Hasta 2 concursos inmobiliarios activos.
	 *
	 * @return array<int, array<string, mixed>>
	 */
