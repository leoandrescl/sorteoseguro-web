<?php
/**
 * Plantilla Autos – página ID 300
 * Misma estructura/CSS que Propiedades; cambia hero, copy de pasos y tarjetas.
 */
if (!defined('ABSPATH')) {
	exit;
}
...
$hero_slides = class_exists('SorteoSeguro_Autos') ? SorteoSeguro_Autos::hero_slides() : [];
$vehicles    = class_exists('SorteoSeguro_Autos') ? SorteoSeguro_Autos::contest_cards() : [];
$soon        = class_exists('SorteoSeguro_Autos') ? SorteoSeguro_Autos::coming_soon_cards(max(0, 4 - count($vehicles))) : [];
$cards       = array_merge($vehicles, $soon);
...
<main class="ss-home ss-realty">
	...
	<!-- Hero slider: Jeep + Peugeot -->
	<section class="ss-home-hero" aria-label="Vehículos destacados" data-ss-hero-slider data-ss-hero-interval="5000">
		<div class="ss-home-hero__viewport">
			<?php foreach ($hero_slides as $i => $slide) :
				...
				$title_tag = $i === 0 ? 'h1' : 'h2';
				$cta_url = (string) ($slide['url'] ?? '#ss-autos-concursos');
				?>
				<article
		...
		<?php if (count($hero_slides) > 1) : ?>
			<div class="ss-home-hero__nav">
				<div class="ss-home-hero__dots" role="tablist" aria-label="Banners de vehículos">
					<?php foreach ($hero_slides as $i => $slide) : ?>
						<button
							type="button"
	...
	<!-- Cómo participar -->
	<section class="ss-realty-how" id="ss-autos-how" aria-labelledby="ss-autos-how-title">
		<div class="ss-home__shell">
			<div class="ss-realty-how__head">
				<p class="ss-realty-how__tagline">¿Cómo participar en los concursos vehiculares de Sorteo Seguro?</p>
				<h2 class="ss-realty-how__title" id="ss-autos-how-title">
					Participar es fácil, rápido y 100% digital. En Sorteo Seguro puedes participar por increíbles premios vehiculares de forma transparente y con respaldo legal. Así funciona:
				</h2>
			</div>
			<div class="ss-realty-how__grid">
				<article class="ss-realty-how__step">
					<p class="ss-realty-how__num">PASO 1</p>
					<h3>Adquiere tu participación digital</h3>
					<p>Elige el vehículo que te interesa y adquiere uno o más DigiTickets desde solo $3.000. Todo el proceso es online, rápido y completamente seguro.</p>
				</article>
				<article class="ss-realty-how__step">
					<p class="ss-realty-how__num">PASO 2</p>
					<h3>Sigue el evento en vivo</h3>
					<p>El proceso se transmite en vivo para garantizar transparencia. No es necesario estar conectado: si resultas ganador, nuestro equipo se comunicará contigo para coordinar el proceso correspondiente según las bases legales.</p>
				</article>
				<article class="ss-realty-how__step">
					<p class="ss-realty-how__num">PASO 3</p>
					<h3>¡Si resultas ganador!</h3>
					<p>Si resultas ganador, nos encargamos de coordinar todo el proceso de entrega del vehículo. Te contactaremos para gestionar la documentación y la entrega correspondiente.</p>
				</article>
			</div>
			<div class="ss-realty-how__cta">
				<a class="ss-home-btn ss-home-btn--primary" href="#ss-autos-concursos">
					Ver eventos activos
					<span class="ss-home-btn__ico" aria-hidden="true"><?php echo $ico['arrow']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
				</a>
			</div>
	...
	<!-- Vehículos activos -->
	<section class="ss-home-contests ss-realty-contests" id="ss-autos-concursos" aria-labelledby="ss-autos-contests-title">
		<div class="ss-home__shell">
			<header class="ss-home-contests__head">
				<div class="ss-home-contests__titles">
					<h2 id="ss-autos-contests-title">VEHÍCULOS ACTIVOS</h2>
				</div>
			</header>
				...
				<?php foreach ($cards as $c) :
					...
					<article class="ss-home-contest<?php echo $is_soon ? ' is-soon' : ''; ?>">
						<div class="ss-home-contest__media">
							...
							<?php endif; ?>
							<span class="ss-home-contest__cat"><?php echo esc_html((string) ($c['category'] ?? 'VEHÍCULO')); ?></span>
						...
						<div class="ss-home-contest__body">
							...
							<?php if (!empty($c['meta'])) : ?>
								<p class="ss-home-contest__meta">
									<span aria-hidden="true"><?php echo $ico['car']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
		...
		<div class="ss-home__shell">
			<div class="ss-home-cta__panel">
				...
				<div class="ss-home-cta__action">
					<a class="ss-home-btn ss-home-btn--light" href="#ss-autos-concursos">
